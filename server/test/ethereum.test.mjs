...
const input = {
  ...
  quote: { network: "ethereum", recipient, amountJpy: 1000 },
};
async function fixture(t) {
  ...
  const publicClient = {
    ...
    readContract: async ({ functionName }) =>
      ({
        ...
        paused: false,
        weiPerJpy: 2000000000000n,
        FEE_BPS: 200n,
        cards: [0n, 0n, zero],
        getReceipt: state.sends
          ? { ...record(), ...(state.corrupt ? { amountWei: 1n } : {}) }
          : { sequence: 0n },
      }[functionName]),
  ...
  const config = {
    deployment: { chainId: 1, contract: address, operator: account.address },
    secret: "s".repeat(32),
    privateKey,
    journalDir: dir,
    publicClient,
  ...
  return { state, config, client: createEthereumPayoutClient(config) };
}
test("Ethereum verifies payout receipt and restart reuses signed transaction", async (t) => {
  const { state, config, client } = await fixture(t);
  const r = await client.pay(input);
  assert.equal(r.amountWei, "1960000000000000");
  assert.equal(r.status, "confirmed");
  assert.deepEqual(await createEthereumPayoutClient(config).pay(input), r);
...
test("uncertain Ethereum confirmation retries the same hash without another signature", async (t) => {
  const { state, config, client } = await fixture(t);
  state.pending = true;
  await assert.rejects(() => client.pay(input), /pending/);
  state.pending = false;
  await createEthereumPayoutClient(config).pay(input);
  assert.equal(state.signs, 1);
  assert.equal(state.sends, 1);
});
test("Ethereum fails closed for wrong chain, low treasury and mismatched receipt", async (t) => {
  const { state, client } = await fixture(t);
  state.chain = 6497;
  await assert.rejects(() => client.pay(input), /not mainnet/);
  state.chain = 1;
  state.treasury = 0n;
  await assert.rejects(() => client.pay(input), /more ETH/);
