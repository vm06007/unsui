...
test('Ethereum receipts survive ledger reload and reject another amount', async t => {
  ...
  const eth = {
    ...input,
    quote: createDemoQuote('1100', 1100, 'ethereum', '0x' + '3'.repeat(40)),
  };
  const payout = createLivePayouts({
    file: f,
    networks: ['sui', 'ethereum'],
    authorize: () => true,
    pay: async () => ({
      status: 'confirmed',
      transactionDigest: '0x' + 'a'.repeat(64),
      chainReceiptId: '0x' + 'b'.repeat(64),
      chainNetwork: 'mainnet',
      amountWei: '2156000000000000',
    }),
  });
  const ledger = createDemoLedger(
    {
  ...
  const receipt = await ledger.record(eth);
  assert.equal(receipt.network, 'ethereum');
  assert.equal((await ledger.list())[0].amountWei, '2156000000000000');
  const { decodeDemoReceipts } = require('../build/demoLedger');
  assert.throws(
    () => decodeDemoReceipts(raw.replace('2156000000000000', '1')),
    /unreadable/,
  );
});

test('MJPY receipt retains yen parity and rejects the wrong token', async t => {
  const f = await file(t);
  let raw = null;
  const request = {
    ...input,
    quote: createDemoQuote('1100', 1100, 'mizuhiki', '0x' + '3'.repeat(40)),
  };
  const payout = createLivePayouts({
    file: f,
    networks: ['mizuhiki'],
    ...
    pay: async () => ({
      ...
      chainReceiptId: '0x' + 'b'.repeat(64),
      chainNetwork: 'awaji',
      amountAtomic: '1078000000',
      tokenAddress: '0x78f5f0Ac4EF201618b97638ded959b155c4f4B04',
    }),
  });
  const ledger = createDemoLedger(
    {
  ...
  const [r] = await ledger.list();
  assert.equal(r.estimatedCrypto, '1078');
  assert.equal(r.payoutAsset, 'MJPY');
  const { decodeDemoReceipts } = require('../build/demoLedger');
  assert.throws(
    () =>
      decodeDemoReceipts(
