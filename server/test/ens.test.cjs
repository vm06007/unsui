...
test('ENS rejects invalid names and missing or zero-address records', async () => {
  for (const name of ['bad name.eth', 'kartik.sui', '', null])
    await assert.rejects(resolveEnsName(name), /valid/);
  for (const address of [null, '0x' + '0'.repeat(40), 'bad'])
    await assert.rejects(
      resolveEnsName('test.eth', { getEnsAddress: async () => address }),
      /no usable/,
    );
});
