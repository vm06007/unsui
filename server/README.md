...
Refunds above ¥1,000 require a server-verified World ID proof, including sample-card requests. Up to ¥1,000 does not require a proof. This is a per-request product threshold, not a daily limit or a Japanese legal threshold. World ID provides the current human-check step in UnSui’s Japan-focused compliance and eligibility design. Passport-attribute checks, visitor/resident eligibility and collection of customer names are potential extensions, not implemented backend features. Applicable KYC, AML and registration obligations must be addressed separately. See [why World ID is part of the payout flow](../README.md#why-world-id-is-part-of-the-payout-flow).
