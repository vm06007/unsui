...
function createDashboardAgent({ apiKey = process.env.OPENROUTER_API_KEY, request = fetch } = {}) {
  return async function chat(body) {
    ...
    const { patchSchema, validatePatch } = await import("../shared/dashboard-settings.mjs");
