      ...
      >
        <DialogContent className="scan-dialog" showCloseButton={false}>
          <div className="scanner-top">
            ...
            <DialogTitle>
              {scanner === 'confirm' ? 'Confirm your card' : 'Scan your card'}
            </DialogTitle>
            <Wifi size={21} />
          </div>
            ...
            <>
              <div className="merkle-visual">
                ...
                <div className="merkle-leaves">
                  {['Card', 'Wallet', 'Amount', 'Network'].map((label) => (
                    <span key={label}>{label}</span>
                  ))}
                </div>
              </div>
              <p className="proof-copy">
                Four fields. One fingerprint. Change a field and the proof no
                longer matches.
              </p>
              ...
              <div aria-live="polite">
                {proofStatus === 'valid' && (
                  <p className="proof-result">
                    <Check size={18} /> Receipt, Merkle root and previous link
                    match.
                  </p>
                )}
                {proofStatus === 'changed' && (
                  <p className="proof-result changed">
                    <ShieldCheck size={18} /> Change detected. Adding ¥1 breaks
                    the proof. Your original receipt is unchanged.
                  </p>
                )}
              </div>
              <p className="proof-scope">
                This proof is local to your browser, not an on-chain
                transaction. The native Sui contract uses its own BCS receipt
                format.
              </p>
              <a
                className="text-link"
                href="https://suivision.xyz/txblock/7US1H68KHniKWxcaSM1xddBiS4Gz7e6No2yGBrQ4zLv"
                target="_blank"
                rel="noreferrer"
              >
                See a real mainnet payout <ExternalLink size={14} />
              </a>
            </>
          )}
        </DialogContent>
