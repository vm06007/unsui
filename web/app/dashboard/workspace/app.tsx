...
export function App({
  ...
  const [agentWidth, setAgentWidth] = useSaved('unsui-agent-width', 380);
  ...
  const [view, setView] = useSaved('unsui-page', 'overview'),
    [source, setSource] = useSaved('unsui-filter-source', 'all'),
    [search, setSearch] = useSaved('unsui-filter-search', ''),
    [from, setFrom] = useSaved('unsui-filter-from', ''),
    [to, setTo] = useSaved('unsui-filter-to', ''),
  ...
  return (
    <div
      ...
      className={`app ${agentOpen ? 'agent-open' : ''} header-${layout.header} sidebar-${layout.sidebar} ${layout.compact ? 'sidebar-compact' : ''} ${layout.showDescription ? '' : 'hide-description'} ${layout.showCardDetails ? '' : 'hide-card-details'} width-${layout.contentWidth}`}
    >
      <AgentSidebar
        page={view}
        open={agentOpen}
      ...
      <div className="main-shell">
        <WorkspaceTopbar
          ...
          onLogout={onLogout}
          agentOpen={agentOpen}
          onAgent={() => setAgentOpen(!agentOpen)}
        />
        <main>
          <WorkspaceHeading
            view={view}
          ...
          >
            {pageLoading ? (
              ...
              <>
                ...
                {['orders', 'payouts', 'reconciliation'].includes(view) && (
                  <OrdersExplorer
                    key={view}
                    rows={rows}
                    view={view}
                ...
                {['overview', 'payouts', 'connections'].includes(view) && (
                  <MultiBaasPanel
                    data={feed.multibaas}
          ...
          <footer>
            <span>
              <img src="/unsui-mark.svg" alt="" /> UnSui (雲水) · Clouds & water
            </span>
            <span>
