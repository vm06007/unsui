...
export const treasuryCardDefaults: SettingItem[] = [
  ['available', 'Available balance', false],
  ['queued', 'Queued commitments', false],
  ['payouts', 'Recorded payouts', false],
  ['runway', 'Projected runway', false],
  ['forecast', 'Buffer forecast', true],
  ['assumptions', 'Scenario assumptions', true],
  ['settlement', 'Settlement and liquidity', true],
].map(([id, label, wide]) => ({
  id: String(id),
  label: String(label),
  wide: Boolean(wide),
...
export function TreasuryCardControls() {
  const [stored, setItems] = useSaved<SettingItem[]>(
    'unsui-treasury-cards',
    treasuryCardDefaults,
  );
  const items: SettingItem[] = normalizeItems(stored, treasuryCardDefaults);
  return (
    <ToolbarPopover
      label="Customize cards"
      title="Customize treasury"
      width={390}
      triggerContent={
        <>
          <Settings2 size={15} />
          Customize cards{' '}
          <small>
            {items.filter((x) => x.enabled).length}/{items.length}
          </small>
        </>
      }
    >
      <p className="subtle">
        Drag to reorder, hide cards, or choose their width. Changes save
        immediately.
      </p>
      <ReorderSettings
        items={items}
        onChange={setItems}
        onReset={() => setItems(treasuryCardDefaults)}
        resize
      />
    </ToolbarPopover>
  );
...
export function TreasuryView({ rows, feed }: { rows: Row[]; feed: Feed }) {
  const [storedCards] = useSaved<SettingItem[]>(
    'unsui-treasury-cards',
    treasuryCardDefaults,
  );
  const cards: SettingItem[] = normalizeItems(
    storedCards,
    treasuryCardDefaults,
  );
  ...
  const sandboxBalance = treasury(
      sandbox,
      asset,
      asset === 'ETH' ? 0.05 : opening,
    ),
    ...
    forecast: (
      <section className="card">
        ...
        <div className="forecast-end">
          <strong>
            {isLive && !availableLive ? '—' : number(forecast.ending, 6)}{' '}
            <small>{asset}</small>
          </strong>
          {(!isLive || availableLive) && (
            <Badge value={forecast.ending < 0 ? 'shortfall' : 'covered'} />
          )}
        </div>
        {(!isLive || availableLive) && (
        ...
        <p className="subtle">
          After {days} days · {dailyOrders} orders/day · {yen(avg)} average
          order
        </p>
      </section>
    ),
        ...
        {!isLive && asset === 'SUI' && (
          <label>
            ...
            <input
              ...
              min="0"
              value={opening}
              onChange={(e) => setOpening(Math.max(0, +e.target.value))}
            />
          </label>
        )}
        <div className="small-note">
          Fixed scenario rate: ¥10,000 = 1 SUI; ¥500,000 = 1 ETH; ¥10,000 = 1
          test MIZU. Actual recorded fees remain unchanged. Projected fee value:{' '}
          {yen(forecast.feesJpy)}.
        </div>
      </section>
    ),
    settlement: (
      <section className="card">
        ...
        <div className="responsibilities">
          ...
          <div>
            <b>2 / Pay out</b>
            <p>
              Crypto leaves the prefunded pool to the recipient. Its receipt is
              linked to the UnSui order.
            </p>
          </div>
          <div>
            <b>3 / Replenish</b>
            <p>
              Convert settled yen and replenish the pool. Forecasts above assume
              no automatic conversion or top-up.
            </p>
          </div>
        </div>
      </section>
  ...
  return (
    <>
      ...
      <div className="custom-card-grid treasury-card-grid">
        {cards
