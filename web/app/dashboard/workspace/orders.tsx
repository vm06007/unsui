...
export const columnDefaults: SettingItem[] = [
  ['id', 'Order'],
  ['date', 'Date (JST)'],
  ['source', 'Source'],
  ['jpy', 'Purchase'],
...
export function OrdersExplorer({
  ...
  const [stored, setColumns] = useSaved<SettingItem[]>(
      `unsui-table-${view}-columns`,
      readSaved('unsui-table-columns', columnDefaults),
    ),
    [mode, setMode] = useSaved(
      `unsui-table-${view}-view`,
      readSaved('unsui-table-view', 'table'),
    ),
    [density, setDensity] = useSaved(
      `unsui-table-${view}-density`,
      readSaved('unsui-table-density', 'comfortable'),
    ),
    [size, setSize] = useSaved(
      `unsui-table-${view}-pagesize`,
      readSaved('unsui-table-pagesize', 10),
    ),
    [sort, setSort] = useSaved(
      `unsui-table-${view}-sort`,
      readSaved('unsui-table-sort', { key: 'date', dir: 'desc' }),
    ),
    [hiddenStatuses, setHiddenStatuses] = useSaved<string[]>(
      `unsui-table-${view}-statuses`,
      [],
    ),
    [hiddenAssets, setHiddenAssets] = useSaved<string[]>(
      `unsui-table-${view}-assets`,
      [],
    ),
    [min, setMin] = useSaved(`unsui-table-${view}-min`, ''),
    [max, setMax] = useSaved(`unsui-table-${view}-max`, ''),
  ...
  const assets = Array.from(
    new Set(['SUI', 'ETH', 'MJPY', ...rows.map((r) => r.asset)]),
  );
  ...
  const columns: SettingItem[] = normalizeItems(stored, columnDefaults);
    ...
    () =>
      rows
        ...
        .sort((a, b) => {
          ...
          return (sort.dir === 'asc' ? 1 : -1) * c || a.id.localeCompare(b.id);
        }),
      ...
      <div className="explorer-toolbar">
        ...
        <div className="table-actions">
              ...
              <fieldset>
                ...
                {statuses.map((key) => (
                  <button
                    ...
                    onClick={() =>
                      setHiddenStatuses(toggle(hiddenStatuses, key))
                    }
                  >
                    <span>
              ...
              <fieldset>
                ...
                {assets.map((key) => (
                  <button
                    ...
                    onClick={() => setHiddenAssets(toggle(hiddenAssets, key))}
                  >
                    <span>
                      {key}{' '}
                      <small>
              ...
              <fieldset>
                ...
                <div className="dual-range">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setMin(String(Math.min(Number(e.target.value), upper)))
                    }
                  />
                  <input
                    type="range"
                    ...
                    value={upper}
                    onChange={(e) =>
                      setMax(String(Math.max(Number(e.target.value), lower)))
                    }
                  />
                </div>
              </fieldset>
          ...
          <ColumnSettings
            columns={columns}
            onChange={setColumns}
            onReset={() => setColumns(columnDefaults)}
          />
          <button
